...
namespace SwfDotNet.IO.Tags.Types
{
	...
	public class Rect: SizeStruct, ISwfSerializer
	{
        ...
        public RectangleF RectangleF
        {
            get
            {
                float xMinPix = (float)xMin / (float)20;
                float yMinPix = (float)yMin / (float)20;
                float xMaxPix = (float)xMax / (float)20;
                float yMaxPix = (float)yMax / (float)20;
                return RectangleF.FromLTRB(xMinPix, yMinPix, xMaxPix, yMaxPix);
            }
        }
